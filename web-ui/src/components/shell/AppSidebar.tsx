  ...
  return (
    <aside className={`${mobileOpen ? "fixed inset-y-0 left-0 z-40 flex" : "hidden md:flex"} h-full w-[17rem] shrink-0 flex-col border-r border-sidebar-border/60 bg-sidebar text-sidebar-foreground`} aria-label="主导航">
      <div className="flex h-14 items-center gap-2 px-4">
        ...
        {onCloseMobile ? <Button variant="ghost" size="icon" className="ml-auto h-7 w-7 rounded-lg md:hidden" aria-label="关闭导航" onClick={onCloseMobile}><X className="h-3.5 w-3.5" /></Button> : null}
      </div>
      ...
      {isAssistant ? <div className="mt-4 min-h-0 flex-1 border-t border-sidebar-border/50 px-2 pt-3">
        <Button variant="ghost" className="mb-2 h-8 w-full justify-start gap-2 rounded-full px-3 text-[12.5px] font-medium" onClick={() => void newConversation()} disabled={loading || !token}><SquarePen className="h-4 w-4" />新建对话</Button>
      ...
      <div className="border-t border-sidebar-border/50 p-3">
        ...
        <div className="flex items-center justify-between">
          <Button variant="ghost" size="icon" title="切换主题" className="h-8 w-8 rounded-lg" aria-label="切换主题" onClick={onToggleTheme}><Sun className="h-4 w-4 dark:hidden" /><Moon className="hidden h-4 w-4 dark:block" /></Button>
          <Button variant="ghost" title="退出登录" className="h-8 gap-2 rounded-full px-3 text-xs text-muted-foreground" aria-label="退出登录" onClick={onLogout}><LogOut className="h-4 w-4" />退出</Button>
        </div>
      </div>
    </aside>
  );
