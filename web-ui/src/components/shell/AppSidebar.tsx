...
type AppSidebarProps = { user: { name: string; roles: string[] }; sessions?: ShellSession[]; activeId?: string | null; onSelectSession?: (id: string) => void; onDeleteSession?: (id: string) => void; onLogout: () => void; onToggleTheme: () => void };
const navItems = [{ to: "/assistant", label: "智能助理", icon: Bot }, { to: "/workflows", label: "工作流", icon: Network }, { to: "/knowledge", label: "知识库", icon: Database }];

export function AppSidebar({ user, sessions = [], activeId = null, onSelectSession = () => {}, onDeleteSession = () => {}, onLogout, onToggleTheme }: AppSidebarProps) {
  return <aside className="flex h-full w-64 shrink-0 flex-col border-r border-border bg-sidebar" aria-label="主导航">
    <div className="flex h-14 items-center px-4 text-base font-semibold">Know-Agent</div>
    ...
    <div className="border-t border-border p-3"><div className="mb-2 truncate px-2 text-sm font-medium" title={user.name}>{user.name}</div><div className="flex items-center justify-between"><button type="button" className="rounded-md p-2 text-muted-foreground hover:bg-sidebar-accent hover:text-sidebar-foreground" aria-label="切换主题" onClick={onToggleTheme}><Sun className="size-4 dark:hidden" aria-hidden="true" /><Moon className="hidden size-4 dark:block" aria-hidden="true" /></button><button type="button" className="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm text-muted-foreground hover:bg-sidebar-accent hover:text-sidebar-foreground" aria-label="退出登录" onClick={onLogout}><LogOut className="size-4" aria-hidden="true" />退出</button></div></div>
  </aside>;
}
