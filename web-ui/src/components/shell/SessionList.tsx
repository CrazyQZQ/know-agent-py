...
export function SessionList({ sessions, activeId, onSelect, onDelete }: SessionListProps) {
  ...
  return <div className="space-y-0.5" aria-label="会话列表">{sessions.map((session) => <div key={session.id} className={`group flex items-center gap-1 rounded-[12px] transition-colors ${activeId === session.id ? "bg-sidebar-accent shadow-[inset_0_0_0_1px_hsl(var(--sidebar-border)/0.45)]" : "hover:bg-sidebar-accent/70"}`}>
    <Button type="text" className="h-8 min-w-0 flex-1 justify-start gap-2 truncate px-3 text-left text-[12.5px]" onClick={() => onSelect(session.id)} aria-current={activeId === session.id ? "page" : undefined}>
    ...
    <Button type="text" className="mr-1 rounded-lg p-1 text-muted-foreground opacity-0 transition-opacity hover:bg-destructive/10 hover:text-destructive group-hover:opacity-100 focus:opacity-100" aria-label={`删除${session.title}`} onClick={() => onDelete(session.id)} icon={<Trash2 className="h-3.5 w-3.5" />} />
  </div>)}</div>;
}
