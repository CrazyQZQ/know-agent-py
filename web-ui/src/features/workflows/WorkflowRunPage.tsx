...
function readUpdateResult(values: Record<string, unknown> | undefined): string {
  if (!values) return "";
  const resultEntry = Object.entries(values).find(([key, value]) =>
    (key === "result" || key.endsWith("_result")) && typeof value === "string",
  );
  return typeof resultEntry?.[1] === "string" ? resultEntry[1].trim() : "";
}

function genericStepMessage(values: Record<string, unknown> | undefined): string | null {
  if (!values) return null;
  const textEntry = Object.entries(values).find(([key, value]) =>
    key !== "result" && !key.endsWith("_result") && typeof value === "string" && value.trim(),
  );
  return typeof textEntry?.[1] === "string" ? textEntry[1].trim() : null;
}
...
function stepMessage(update: StepUpdate): string | null {
  ...
  return genericStepMessage(values);
}
...
export function WorkflowRunPage() {
  ...
  const stepDefinitions = useMemo(() => graphNodes.map((node) => ({
    id: node.id,
    label: STEP_PRESENTATION[node.id]?.label ?? node.name,
    description: STEP_PRESENTATION[node.id]?.description ?? "工作流节点",
  })), [graphNodes]);
  ...
  const activeIndex = status === "waiting"
    ? clarificationIndex
    : status === "running"
      ? Math.min(lastCompletedIndex + 1, stepDefinitions.length - 1)
      : lastCompletedIndex;
  ...
  useEffect(() => {
    ...
    setPageError("");
    void Promise.all([
      getGraphTopology(workflowId, auth?.token),
      listGraphs(auth?.token),
    ]).then(
      ([topology, graphs]) => {
        if (cancelled) return;
        ...
        if (topology.nodes.length > 0) {
          setSelectedNode((current) => topology.nodes.some((node) => node.id === current)
            ? current
            : topology.nodes[0].id);
        }
      },
      (error) => {
        if (!cancelled) {
          setPageError(error instanceof ApiError && error.status === 404
            ? "工作流不存在或已下线"
            : "工作流信息加载失败，请稍后重试。");
        }
      },
  ...
  function consume(event: SseEvent) {
    if (event.event === "update") {
      try {
        ...
        const updateResult = readUpdateResult(update.values);
        if (updateResult) setResult(updateResult);
      } catch { /* ignore malformed update */ }
    } else if (event.event === "done") {
      setStatus("done");
      const resultNode = graphNodes.at(-1)?.id ?? "result";
      setSelectedNode(resultNode);
      try {
        const nextResult = String((JSON.parse(event.data) as { result?: string }).result ?? "");
        setResult(nextResult);
        if (nextResult) {
          const isPpt = workflowId === "ppt_build";
          const content = isPpt
            ? `PPT 已生成。\n\n[下载演示文稿](${nextResult})`
            : `工作流已完成。\n\n[下载结果](${nextResult})`;
          setMessages((current) => current.some((message) => message.node === resultNode)
            ? current
            : [...current, createMessage("assistant", content, resultNode)]);
        }
  ...
  return <div className="relative flex h-full min-h-0 flex-col bg-background">
    <header className="flex h-16 shrink-0 items-center gap-3 border-b border-border/55 px-5">
      ...
      <div className="min-w-0"><h1 className="truncate text-[15px] font-semibold">{workflowTitle}</h1><p className="text-xs text-muted-foreground">引导式生成工作台</p></div>
