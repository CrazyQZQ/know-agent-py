...
export function KnowledgeListPage() {
  ...
  const load = useCallback(async () => {
    setRows(await listAllDocuments(auth?.token ?? ""));
  }, [auth?.token]);
  useEffect(() => {
    void load();
  }, [load]);
  const remove = async (id: number) => {
    await deleteDocument(auth?.token ?? "", id);
    setRows((prev) => prev.filter((row) => row.id !== id));
  };
  const filtered = rows.filter(
    (row) =>
      (!query || row.title.toLowerCase().includes(query.toLowerCase())) &&
      (filter === "all" || row.knowledge_base_type === filter),
  );
        ...
        <tbody>
          {visible.map((row) => (
            <tr key={row.id} className="border-b">
              ...
              <td className="p-2">
                <span
                  className={`rounded px-2 py-1 text-xs ${STATUS[row.status] ?? STATUS.UPLOADED}`}
                >
                  {row.status}
                </span>
              </td>
